...
def creer_connection(db_fichier):
    ...
    """
    conn=None
    conn = sqlite3.connect(db_fichier, check_same_thread=False)
...
def recherche_pokemons():
    ...
    cur=bdd.cursor()
    cur.execute('SELECT nom FROM Pokemon Where nom IS NOT "NULL"')
    bdd.commit()
    ListePokemon=cur.fetchall()
    ListePokemon.sort()
    return ListePokemon

# fonction de recherche du nombre de Pokemons dans le pokedex
def nombre_pokemons():
    return len(recherche_pokemons())
...
def recherche_attributs_pokemon(nom_pokemon):
    ...
    print(nom_pokemon)
    class Pokemon():
        def __init__(self,ids,name,nom,pv,attaque,defense,attaque_speciale,defense_speciale,vitesse,evolution_de,description):
            self.id=ids
            self.name=name
            self.nom=nom
            self.pv=pv
    ...
    if type(nom_pokemon)==tuple:
            cur.execute("Select * From pokemon where nom=?",[nom_pokemon[0]])
            liste=cur.fetchall()
    else:
        print(nom_pokemon)
        print(nom_pokemon)
        cur.execute("Select * From pokemon where nom=?",[nom_pokemon])
        liste=cur.fetchall()
    print(liste)
    
    if liste[0][9]!=None:
        cur.execute(f"Select nom From pokemon where id={liste[0][9]}")
        liste2=cur.fetchall()
        print(liste2)
        pokemon=Pokemon(liste[0][0],liste[0][1],liste[0][2],liste[0][3],liste[0][4],liste[0][5],liste[0][6],liste[0][7],liste[0][8],liste2[0][0],liste[0][10])
    else:
         pokemon=Pokemon(liste[0][0],liste[0][1],liste[0][2],liste[0][3],liste[0][4],liste[0][5],liste[0][6],liste[0][7],liste[0][8],'...',liste[0][10])       
...
def recherche_evolutions(nom_pokemon):
    ...
    a=cur.fetchone()
    print(a)
    cur.execute('Select nom from pokemon where evolution_de=?',[a[0]])
    listeEvo=cur.fetchall()
...
def recherche_types(nom_pokemon):
    ...
    cur=bdd.cursor()
    print(nom_pokemon)
    cur.execute('Select type.nom from type Inner Join pokemon on pokemon.id=est_de_type.id_pokemon inner join est_de_type on est_de_type.id_type=type.id  Where pokemon.nom=?',[nom_pokemon[0]])
    
    r=cur.fetchall()
    listeType=[]
    for i in range(len(r)):
        listeType.append(r[i][0])
    print(listeType)
    return listeType
#                                                          #
#                                                          #
#              FIN DE LA  PARTIE ELEVE                     #
# ##########################################################   
